...
public class Service {
    ...
    public void dropAbuses(InputStream charIn, OutputStream charOut, String[] abuse) throws IOException {
        try (BufferedReader charsInReadBuff = new BufferedReader(new InputStreamReader(charIn));
        BufferedWriter charOutWriteBuff = new BufferedWriter(new OutputStreamWriter(charOut))) {
            String str = charsInReadBuff.readLine();
            for (String substr : abuse) {
                if (str.contains(substr)) {
                    str = str.replace(substr, "");
                }
            }
            charOutWriteBuff.write(str);
        }
    }
